...
public class KeepAliveScheduler {
  ...
  public void registerSessionKeepAlive(WebSocketSession session) {
    lock.writeLock().lock();
    sessionScheduledKeepAliveTasks.put(
      session.getId(),
      executorService.scheduleAsyncTask(
  ...
  private Runnable keepAliveTask(WebSocketSession session) {
    return () -> {
      if (!sessionScheduledKeepAliveTasks.containsKey(session.getId())) return;
      try {
        if (session.isOpen()) session.sendMessage(MessagingFactory.keepAlive());
