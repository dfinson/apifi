...
public class SseSubscriptionsHandler {
  ...
  public SseEmitter handleSubscriptionRequest(
    GraphQLRequest graphQLRequest,
    Long timeoutParam,
    HttpServletRequest httpServletRequest
  ) {
    log.info("Parsing subscription request");
    val timeout = configValues.getSseTimeoutParamEnabled() &&
      timeoutParam != null
      ? timeoutParam
      : configValues.getSseTimeout();
    val emitter = new SseEmitter(timeout);
    val executionResult = requestExecutor.executeQuery(
      graphQLRequest,
      httpServletRequest
    );
    if (
      !(executionResult.getData() instanceof Publisher)
    ) throw new RuntimeException(
      "SSE endpoint is intended for use with subscriptions only"
    );
