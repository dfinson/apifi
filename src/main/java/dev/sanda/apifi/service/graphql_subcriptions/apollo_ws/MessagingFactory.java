...
public class MessagingFactory {
  ...
  public static final String GRAPHQL_CONNECTION_TERMINATE =
  ...
  public static final OperationMessage CONNECTION_ACK = new OperationMessage(
    GRAPHQL_CONNECTION_ACK
  );
  public static final OperationMessage KEEP_ALIVE = new OperationMessage(
    GRAPHQL_CONNECTION_ACK
  );

  private static final Class<PayloadMessage> PAYLOAD_MESSAGE_CLASS =
    PayloadMessage.class;
  private static final Class<OperationMessage> OPERATION_MESSAGE_CLASS =
  ...
  private void initMapper() {
    ...
    module
      .addDeserializer(
        OPERATION_MESSAGE_CLASS,
        new JsonDeserializer<OperationMessage>() {
          @Override
          @SneakyThrows
          public OperationMessage deserialize(
      ...
      .addSerializer(
        OPERATION_MESSAGE_CLASS,
        new JsonSerializer<OperationMessage>() {
          @Override
          @SneakyThrows
      ...
      .addDeserializer(
        PAYLOAD_MESSAGE_CLASS,
        new JsonDeserializer<PayloadMessage>() {
          @Override
          @SneakyThrows
          public PayloadMessage deserialize(
            JsonParser jsonParser,
            DeserializationContext deserializationContext
          ) {
            val node = (JsonNode) jsonParser.getCodec().readTree(jsonParser);
            val apolloPayloadMessage = new PayloadMessage<>(
              (Object) node.get("payload")
            );
            apolloPayloadMessage.setId(
              node.has("id") ? node.get("id").asText() : null
            );
            apolloPayloadMessage.setType(
              node.has("type") ? node.get("type").asText() : null
            );
      ...
      .addSerializer(
        PAYLOAD_MESSAGE_CLASS,
        new JsonSerializer<PayloadMessage>() {
          @Override
          @SneakyThrows
          public void serialize(
            PayloadMessage apolloPayloadMessage,
            JsonGenerator gen,
            SerializerProvider serializerProvider
          ) {
            gen.writeStartObject();
  ...
  public static TextMessage connectionError(final String message)
  ...
  public static TextMessage data(String id, ExecutionResult result)
    throws JsonProcessingException {
    return jsonMessage(
      new PayloadMessage<>(id, GRAPHQL_DATA, result.toSpecification())
    );
  }
  ...
  public static TextMessage error(String id, List<GraphQLError> errors)
    throws JsonProcessingException {
    val errorMap = errors
      .stream()
      .filter(
  ...
  public static TextMessage error(String id, Throwable exception)
    throws JsonProcessingException {
    return error(id, exception.getMessage());
  }

  public static TextMessage error(String id, String message)
    throws JsonProcessingException {
    val errorMap = Collections.singletonList(
      Collections.singletonMap("message", message)
    );
    return jsonMessage(new PayloadMessage<>(id, GRAPHQL_ERROR, errorMap));
  }

  private static TextMessage jsonMessage(OperationMessage message)
    throws JsonProcessingException {
