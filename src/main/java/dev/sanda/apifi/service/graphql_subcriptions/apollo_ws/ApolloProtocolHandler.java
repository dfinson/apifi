...
public class ApolloProtocolHandler
  extends TextWebSocketHandler
  implements ApolloSubProtocolCapable {
  ...
  public void afterConnectionEstablished(WebSocketSession session) {
    if (
      configValues.getWsKeepAliveEnabled()
  ...
  protected void handleTextMessage(
    ...
    try {
      ...
      switch (apolloMessage.getType()) {
        case GRAPHQL_CONNECTION_INIT:
          handleInit(session);
          break;
        case GRAPHQL_START:
          handleStart(session, apolloMessage);
          break;
        case GRAPHQL_STOP:
          handleStop(apolloMessage);
          break;
        case GRAPHQL_CONNECTION_TERMINATE:
  ...
  private void handleTerminate(WebSocketSession session) {
    log.info(
      "Terminating Apollo Protocol GraphQLSubscription on WebSocketSession #" +
      session.getId()
    );
    session.close();
  }
  ...
  private void handleStop(OperationMessage operationMessage) {
    ...
    if (toStop != null) {
      log.info(
        "Stopping Apollo Protocol GraphQLSubscription #" + toStop.getApolloId()
      );
  ...
  private void handleStart(
    ...
    log.info(
      "Starting Apollo Protocol GraphQLSubscription in WebSocketSession #" +
      session.getId() +
      " with message: " +
      operationMessage.toString()
    );
    val request = GraphQLRequest.fromObjectNode(
      ((PayloadMessage<ObjectNode>) operationMessage).getPayload()
    );
    val result = executor.executeQuery(request, session);
    if (result.getData() instanceof Publisher) handleSubscription(
      operationMessage.getId(),
      result,
      session
    ); else handleQueryOrMutation(operationMessage.getId(), result, session);
  }
  ...
  private void handleInit(WebSocketSession session) {
    log.info(
      "Initializing Apollo Protocol GraphQLSubscription in WebSocketSession #" +
      session.getId()
    );
  ...
  @SneakyThrows
  private OperationMessage getApolloMessage(
    WebSocketSession session,
    TextMessage message
  ) {
    try {
  ...
  public static void fatalError(WebSocketSession session, Exception exception) {
    try {
      log.error(
        "Encountered fatal error during session \"" +
        session.getId() +
        "\" - closing the session with status 'SESSION_NOT_RELIABLE'"
      );
    ...
    log.warn(
      String.format(
        "WebSocket session %s (%s) closed due to an exception",
        session.getId(),
  ...
  private void addSubscription(
    String apolloId,
    ApolloSubscriber subscriber,
    Publisher<ExecutionResult> publisher
  ) {
    subscriptions.put(
      apolloId,
      new ApolloSubscription(apolloId, subscriber, publisher)
    );
  }
}
