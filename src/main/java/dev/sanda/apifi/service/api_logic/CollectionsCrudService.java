...
public class CollectionsCrudService<T> extends BaseCrudService<T> {
    ...
    public <TCollection, E extends EntityCollectionApiHooks<TCollection, T>> CompletionStage<List<List<TCollection>>> getEntityCollectionImpl(
            List<T> input,
        ...
        BatchLoader<T, List<TCollection>> batchLoader = keys -> {
            if (collectionApiHooks != null) keys.forEach(
                    owner ->
        ...
        DataLoader<T, List<TCollection>> dataLoader = DataLoader.newDataLoader(batchLoader);
    ...
    public <TCollection> CompletionStage<List<TCollection>> getEmbeddedImpl(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            @SuppressWarnings("unchecked") final Map<T, TCollection> resultMap = (Map<T, TCollection>) dataManager
                    .entityManager()
