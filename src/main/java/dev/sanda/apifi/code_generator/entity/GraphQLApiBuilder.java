...
public class GraphQLApiBuilder {
  ...
  public ServiceAndTestableService build(
    ...
    if (!methodLevelSecurities.isEmpty()) methodLevelSecurities.forEach(
      security -> handleTargetMethodsMapping(security, methodLevelSecuritiesMap)
    );
        ...
        fieldApiSpec -> {
          ...
          if (isIterable(fieldElement.asType(), processingEnv)) {
            ...
            if (resolvers.contains(GET_PAGINATED__BATCH)) {
              ...
              clientQueryBuilder.setOwnerEntityType(entityName);
              val getPaginatedBatchInEntityCollection = genGetPaginatedBatchInEntityCollection(
                fieldApiSpec,
                clientQueryBuilder
              );
            ...
            if (resolvers.contains(PAGINATED__FREE_TEXT_SEARCH)) {
              ...
              clientQueryBuilder.setOwnerEntityType(entityName);
              val getPaginatedFreeTextSearchInEntityCollection = genGetPaginatedFreeTextSearchInEntityCollection(
                fieldApiSpec,
                clientQueryBuilder
              );
              serviceBuilder.addMethod(
  ...
  private void generateMapElementCollectionMethods(
    ...
    if (endpoints.contains(PAGINATED__BATCH__)) {
      ...
      clientQueryBuilder.setOwnerEntityType(entityName);
      final MethodSpec getPaginatedBatchInMapElementCollection = genGetPaginatedBatchInMapElementCollection(
        elemCollectionSpec,
        clientQueryBuilder
      );
    ...
    if (endpoints.contains(PAGINATED__FREE__TEXT__SEARCH)) {
      ...
      clientQueryBuilder.setOwnerEntityType(entityName);
      val freeTextSearchInMapElementCollection = genFreeTextSearchInMapElementCollection(
        elemCollectionSpec,
        clientQueryBuilder
      );
  ...
  public FieldSpec entityCollectionApiHooks(VariableElement fk) {
    ...
    return FieldSpec
      .builder(apiHooksType, entityCollectionApiHooksName(fk), Modifier.PRIVATE)
      .addAnnotation(Autowired.class)
      .build();
  }
      ...
      .filter(
        elem ->
          ...
          elem
            .getSimpleName()
            .toString()
            .equals("get" + pascalCaseNameOf(fk)) &&
  ...
  private void registerCollectionsTypes(
    ...
    this.fieldGraphQLApiSpecs.forEach(
        fieldGraphQLApiSpec -> {
          val fieldElement = fieldGraphQLApiSpec.getElement();
          if (isIterable(fieldElement.asType(), processingEnv)) {
            String key =
              apiSpec.getSimpleName() +
              "." +
              fieldGraphQLApiSpec.getSimpleName();
