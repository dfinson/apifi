...
public class GraphQLQueryBuilder {
  ...
  public String args() {
    ...
    vars.forEach(
      (varName, varType) -> {
        ...
        if (!isSubscription()) builder.append(", ");
      }
    );
  ...
  private String resolveVarTypescriptType(String varName) {
    ...
    return isSubscription()
      ? resolveSubscriptionVarTypescriptType(varName)
      : resolveQueryOrMutationVarTypescriptType(varName);
  }

  private String resolveSubscriptionVarTypescriptType(String varName) {
    switch (subscriptionObservableType) {
      case LIST_TO_OBSERVE:
        return String.format(
          ": SubscriptionRequestInput<%s>",
          entityReturnType
        );
      case COLLECTION_OWNER:
        return String.format(
          ": EntityCollectionSubscriptionRequestInput<%s, %s>",
          ownerEntityType,
          entityReturnType
        );
      case ENTITY_TYPE:
        return String.format(
          ": BaseSubscriptionRequestInput<Array<%s>>",
          entityReturnType
        );
    }
  ...
  private String resolveQueryOrMutationVarTypescriptType(String varName) {
    if (varName.equals("owner")) return ": " + this.ownerEntityType;
    if (queryName.contains("ByUnique")) return ": " + findByUniqueFieldType;
    switch (clientSideReturnType) {
      case PAGE:
        return (
          ": " +
          (
            isFreeTextSearchQuery()
      ...
      case ARRAY:
        return ": Array<" + entityReturnType + ">";
      case SET:
        return ": Set<" + entityReturnType + ">";
      case MAP:
        return queryName.startsWith("add")
          ? ": Map<" + entityReturnType + ">"
          : ": Array<" +
          entityReturnType.substring(0, entityReturnType.indexOf(",")) +
          ">";
    }
    return ": any";
  }
  ...
  private String subscriptionVarsDef() {
    ...
    switch (subscriptionObservableType) {
      case LIST_TO_OBSERVE:
        builder
          .append("$toObserve: ")
          .append(String.format("[%s]", entityReturnType))
          .append(", ");
        break;
      case COLLECTION_OWNER:
        builder.append("$owner: ").append(ownerEntityType).append(", ");
        break;
    }
  ...
  private boolean isEntityType(String varType) {
    return entityTypes.contains(
      varType.replaceAll("\\[", "").replaceAll("]", "")
    );
  }

  private String varsArgs() {
    if (vars.isEmpty()) return "";
    if (isSubscription()) return subscriptionVarsArgs();
    val builder = new StringBuilder();
    vars.forEach(
      (varName, varType) ->
        builder.append(varName).append(": $").append(varName).append(", ")
    );
    builder.setLength(builder.length() - 2);
    return "(" + builder + ")";
  }
  ...
  private String varsVals() {
    if (vars.isEmpty()) return "";
    if (isSubscription()) return subscriptionVarsVals();
    val builder = new StringBuilder();
    vars.forEach(
      (varName, varType) ->
        builder
  ...
  private String subscriptionVarsVals() {
    val builder = new StringBuilder();
    if (subscriptionObservableType.equals(LIST_TO_OBSERVE)) builder
      .append("\t")
      .append(inQuotes("toObserve"))
      .append(": ")
      .append("input.toObserve")
      .append(", \n\t\t\t\t\t"); else if (
      subscriptionObservableType.equals(COLLECTION_OWNER)
    ) builder
      .append("\t")
  ...
  private boolean isFreeTextSearchQuery() {
    return vars
      .getOrDefault("input", "")
      .equals("FreeTextSearchPageRequestInput");
  }
  ...
  public String getSubscriptionReturnType() {
    return subscriptionObservableType.equals(LIST_TO_OBSERVE)
      ? entityReturnType
      : "Array<" + entityReturnType + ">";
  }
}
